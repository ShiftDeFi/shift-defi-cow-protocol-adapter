...
2. **Rule out a non-finding.** An aderyn key is anchored by line, so inserting a
   line above a reviewed finding renumbers it: `make retriage --check` says
   whether that is all that happened. A finding that appears locally but not in
   CI is usually a version mismatch — `make tools` against the pins in
...
5. **Decide.** Premise holds → fix the code. Premise fails → propose an entry,
   with the fact from step 4 as the reasoning.
...
1. The key — `make aderyn` prints the un-triaged ones.
